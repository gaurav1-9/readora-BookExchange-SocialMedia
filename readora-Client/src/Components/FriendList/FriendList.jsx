...
const FriendList = ({ list, loading }) => {
    return (
        <div className='w-8/9 mt-3 bg-gunMetal rounded-xl p-4 '>
                    ...
                    list.map((item, index) => (
                        <div key={index} className='flex p-2 rounded-xl justify-between items-center'>
                            <div className='flex' onClick={() => console.log(item)}>
                                <img src={item.profilePic} alt="Friend" className="w-10 h-10 rounded-full cursor-pointer" draggable='false' />
            ...
            {
            list.length ? <div className='mt-5 flex justify-end'>
