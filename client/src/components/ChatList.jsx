...
const ChatList = ({chats, user}) => {
    return (
        <ul>
            {
                chats.map(chat => (
                    <li key={chat._id} className='flex gap-2 items-center bg-gunMetal/10 rounded-lg px-4 py-2 cursor-pointer hover:bg-gunMetal/14'>
                        ...
                        <div className="flex flex-col w-full">
                            <div className="flex justify-between items-center">
                                {
                                    chat.users.map((chatUser) => (
                                        (chatUser._id.toString() === user._id)
                                            ? null
                                            : <p className='font-semibold text-xl text-gunMetal'>{chatUser.username}</p>
                                    ))
                                }
                            ...
                            <div className="flex items-center gap-2">
                                ...
                                <p className='text-md text-gunMetal/90'>{chat.latestMessage.content}</p>
                            </div>
